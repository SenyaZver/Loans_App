...
class LoanDetailViewModel(private var repository: ChosenLoanRepository, private var login:String): ViewModel() {
    ...
    private var loanAmount = MutableLiveData<Double>(repository.getChosenLoan()?.amount ?: 0.0)
    private var loanAmountLeft = MutableLiveData<Double>(repository.getChosenLoan()?.amount_left ?: 0.0)
    ...
    fun getAmountLeft(): LiveData<Double> {
